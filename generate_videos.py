...
def save_images(image_urls, output_dir, celebrity):
    ...
    for i, url in enumerate(image_urls):
        try:
            ...
            logging.info(f"Saved image {i+1}/{len(image_urls)}: {image_path}")
...
def create_video(image_paths, output_path, celebrity, audio_path=None, duration_per_image=4.3, title="", description="", script=""):
    """Create a video from images with optional audio."""
    try:
        clips = []
...
def generate_single_video(celebrity, output_dir, title, description, script, video_url=None):
    """Generate a single video for a celebrity."""
    logging.info(f"Generating video for {celebrity}")
    image_urls = download_images(celebrity)
    image_paths = save_images(image_urls, output_dir, celebrity)
    
    video_filename = f"{celebrity.lower().replace(' ', '-')}-history.mp4"
    video_path = os.path.join(output_dir, video_filename)
    
    audio_path = generate_audio_from_script(script, output_dir)
    try:
